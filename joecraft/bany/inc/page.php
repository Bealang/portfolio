...
//        return "404";
    ...
    function print_header($container_start = true, $title = null, $class = "litebans-header") {
        $table = $this->name;
        if ($title === null) {
            $title = $this->title;
        }
        ...
        <h1>'. $title . '</h1>
            <div class="litebans-check">
                ...
                <script type="text/javascript">function captureForm(b){var o=$(".litebans-check-output");o.removeClass("show");var x=setTimeout(function(){o.html("")}, 150);$.ajax({type:"GET",url:"check.php?name="+$("#user").val()+"&table=' . $table . '"}).done(function(c){clearTimeout(x);o.html(c);o.addClass("show")});b.preventDefault();return false};</script>
            </div>
        </div>
    ...
    function table_begin() {
        echo '<div class="sleek-content">';
        $this->print_navbar(array(
            "bans.php"     => $this->t("title.bans"),
            "mutes.php"    => $this->t("title.mutes"),
            "warnings.php" => $this->t("title.warnings"),
            "kicks.php"    => $this->t("title.kicks"),
        ));
    ...
    function print_navbar($links) {
        echo '<ul class="sleek-navbar">';
        foreach ($links as $page => $title) {
            ...
            if ($this->settings->header_show_totals && isset($count[$page])) {
                $title .= ' <span class="' . $this->settings->badge_classes . '">';
                $title .= $count[$page];
                $title .= "</span>";
            }
